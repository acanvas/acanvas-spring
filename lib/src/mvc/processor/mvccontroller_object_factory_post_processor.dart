...
/**
	 ...
	 *  class SomeCommandType {
	 * //implementation ommitted...
	 * }
	 * </pre>
	 * <p>If the command needs to be excecuted after a specific event <code>Type</code> was dispatched then add this
	 * metadata:</p>
	 * <pre>
	 * [Command(eventType="com.events.MyEvent")]
	 *  class SomeCommandType {
	 * //implementation ommitted...
	 * }
	 * </pre>
...
class MVCControllerObjectFactoryPostProcessor extends AbstractOrderedFactoryPostProcessor {
  ...
  /*
		  void registerCommands(IObjectFactory objectFactory,IController controller) {
			...
			for (String key in objectFactory.objectDefinitionRegistry.objectDefinitionNames) {
				IObjectDefinition def = objectFactory.getObjectDefinition(key);
				Type type = Type.forName(def.className, objectFactory.applicationDomain);
...
*/
  ...
  /*
		  void processCommandMetaData(Type type,IController controller,String commandName,ApplicationDomain applicationDomain) {
			...
			if (type.hasMetadata(COMMAND_METADATA)) {
				List metaData = type.getMetadata(COMMAND_METADATA);
				for (Metadata md in metaData) {
  ...
  /*
		  void processMetaData(Metadata metaData,IController controller,String commandName,ApplicationDomain applicationDomain) {
			...
			int prio = (metaData.hasArgumentWithKey(PRIORITY_METADATA_KEY)) ? parseInt(metaData.getArgument(PRIORITY_METADATA_KEY).value) : 0;

			String executeMethod = (metaData.hasArgumentWithKey(EXECUTE_METHOD_METADATA_KEY)) ? metaData.getArgument(EXECUTE_METHOD_METADATA_KEY).value : DEFAULT_EXECUTE_METHOD_NAME;

			List<String> properties = getPropertiesFromMetaData(metaData);

			if (metaData.hasArgumentWithKey(EVENT_TYPE_METADATA_KEY)) {
				String eventType = metaData.getArgument(EVENT_TYPE_METADATA_KEY).value;
				...
				Type eventType = TypeUtils.forName(metaData.getArgument(EVENT_CLASS_METADATA_KEY).value, applicationDomain);
				controller.registerCommandForEventType(eventType, commandName, executeMethod, properties, prio);
			}
		}
		 */
  ...
  /*
		  List<String> getPropertiesFromMetaData(Metadata metaData) {
			...
			if (metaData.hasArgumentWithKey(PROPERTIES_METADATA_KEY)) {
				String value = metaData.getArgument(PROPERTIES_METADATA_KEY).value;
				List arr = value.split(' ').join('').split(',');
